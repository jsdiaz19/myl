...
import { BudgetGoalService} from '../../service/BudgetGoal/budget-goal.service'
@Component({
...
export class BudgetGoalComponent implements OnInit {
  ...
  id=null;
  constructor(private HttpBD: HttBDService,private DataUsr: DataService, private storeSrvice: ShopsService, private Budget: BudgetGoalService) { }

  ngOnInit() {
    var d = new Date();
    var n = d.getFullYear();
    var m = d.getMonth();
    this.id=this.DataUsr.GetCo().toString().trim(); 
    this.Budget.Budget(n).subscribe(result=>{
    ...
    this.Budget.BudgetMonth(n).subscribe(result=>{
    ...
    this.Budget.BudgetWeek(n,m).subscribe(result=>{
    ...
    this.storeSrvice.ListStore().subscribe(result => {
      this.Store=result;
    })
  }
