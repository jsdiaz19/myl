...
import { CheckService} from '../../service/CheckService/check.service'
...
export class CheckDespachoComponent implements OnInit {
  ...
  refUnd=null; 
  Attempts=2;
  constructor(private _route: ActivatedRoute, private HttpBD: CheckService,private Despacho: CreateDespachoService ,private router: Router,public dialog: MatDialog) { 
  ...
  Add_product(){
    if (document.getElementById('edit').textContent.length==12){
      ...
      this.Despacho.GetReference(content).subscribe(result =>{
        if (result!=null){
          this.numProduct+=1;
